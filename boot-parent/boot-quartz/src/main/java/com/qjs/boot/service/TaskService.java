...
import com.qjs.boot.task.job.ScheduledJob;
...
public class TaskService {
			...
			for (String groupJob : scheduler.getJobGroupNames()) {
				for (JobKey jobKey : scheduler.getJobKeys(GroupMatcher.<JobKey>groupEquals(groupJob))) {
					...
					for (Trigger trigger : triggers) {
						...
						info.setUrl(dataMap.getString("url"));
						list.add(info);
					}
				}
			}
	...
	public void addJob(TaskInfo info) {
		...
		try {
			...
			// Class<? extends Job> clazz = (Class<? extends Job>)
			// Class.forName(jobName);
			JobDetail jobDetail = JobBuilder.newJob(ScheduledJob.class).withIdentity(jobKey)
					.withDescription(jobDescription).usingJobData("url", info.getUrl()).build();
			jobDetail.getJobDataMap().put("scheduleEntity", info);
